...
function (_VideoWrapper) {
  ...
  function ChimeePlugin(_ref, dispatcher) {
    ...
    var id = _ref.id,
        ...
        events$$1 = _ref$events === void 0 ? {} : _ref$events,
        _ref$data = _ref.data,
    ...
    if (!toxicPredicateFunctions.isEmpty(methods) && _isPlainObject(methods)) {
      Object.keys(methods).forEach(function (key) {
        ...
        Object.defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), key, {
          configurable: true,
          enumerable: false,
          value: toxicUtils.bind(fn, _assertThisInitialized(_assertThisInitialized(_this))),
          writable: true
        });
      });
    }

    if (!toxicPredicateFunctions.isEmpty(events$$1) && _isPlainObject(events$$1)) {
      Object.keys(events$$1).forEach(function (key) {
        if (!_isFunction(events$$1[key])) {
          throw new TypeError('plugins events hook must bind with Function');
        }

        _this.$on(key, events$$1[key]);
      });
    }

    if (!toxicPredicateFunctions.isEmpty(data) && _isPlainObject(data)) {
      Object.assign(_assertThisInitialized(_assertThisInitialized(_this)), _cloneDeep(data));
    }
    ...
    if (!toxicPredicateFunctions.isEmpty(computed) && _isPlainObject(computed)) {
      ...
      toxicDecorators.applyDecorators(_assertThisInitialized(_assertThisInitialized(_this)), props, {
        self: true
      });
    }
    ...
    toxicDecorators.applyDecorators(_assertThisInitialized(_assertThisInitialized(_this)), {
...
function (_ChimeePlugin) {
  ...
  function PictureInPicture(config, dispatcher, option) {
    ...
    _this.requestPictureInPicture = function () {
      ...
      window.__chimee_picture_in_picture = {
        element: _this.$video,
        plugin: _assertThisInitialized(_assertThisInitialized(_this)),
        window: _this.$dom
      };
      return Promise.resolve();
    };
...
function (_VideoWrapper) {
  ...
  function Chimee(rawConfig) {
    ...
    _this.dispatcher = new Dispatcher(config, _assertThisInitialized(_assertThisInitialized(_this)));
